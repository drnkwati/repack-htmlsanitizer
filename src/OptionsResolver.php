...
class OptionsResolver implements AspectOptions
{
    /**
     * The fully qualified name of the {@link AspectOptions} interface.
     *
     * @internal
     */
    const OPTIONS_INTERFACE = '\Repack\\HtmlSanitizer\\AspectOptions';
     ...
     * @return $this
     *
     * @throws LogicException If called from a lazy option or normalizer
     */
    public function setDefault($option, $value)
    {
        ...
        if ($this->locked) {
            throw new LogicException('Default values cannot be set from a lazy option or normalizer.');
        }
    ...
    {
        if ($this->locked) {
            throw new LogicException('Options cannot be made required from a lazy option or normalizer.');
        }
    ...
    {
        if ($this->locked) {
            throw new LogicException('Options cannot be defined from a lazy option or normalizer.');
        }
     ...
     * @return $this
     *
     * @throws InvalidArgumentException If the option is undefined
     * @throws LogicException           If called from a lazy option or normalizer
     */
    public function setNormalizer($option, \Closure $normalizer)
    {
        if ($this->locked) {
            throw new LogicException('Normalizers cannot be set from a lazy option or normalizer.');
        }

        if (!isset($this->defined[$option])) {
            throw new InvalidArgumentException(sprintf('The option "%s" does not exist. Defined options are: "%s".', $option, implode('", "', array_keys($this->defined))));
        }
    ...
    {
        if ($this->locked) {
            throw new LogicException('Allowed values cannot be set from a lazy option or normalizer.');
        }
    ...
    {
        if ($this->locked) {
            throw new LogicException('Allowed values cannot be added from a lazy option or normalizer.');
        }
    ...
    {
        if ($this->locked) {
            throw new LogicException('Allowed types cannot be set from a lazy option or normalizer.');
        }
    ...
    {
        if ($this->locked) {
            throw new LogicException('Allowed types cannot be added from a lazy option or normalizer.');
        }
    ...
    {
        if ($this->locked) {
            throw new LogicException('Options cannot be removed from a lazy option or normalizer.');
        }
    ...
    {
        if ($this->locked) {
            throw new LogicException('Options cannot be cleared from a lazy option or normalizer.');
        }
     ...
     *
     * @throws InvalidArgumentException If an option name is undefined
     * @throws InvalidArgumentException   If an option doesn't fulfill the
     *                                   specified validation rules
     * @throws InvalidArgumentException   If a required option is missing
     * @throws LogicException If there is a cyclic dependency between
     *                                   lazy options and/or normalizers
     * @throws OutOfBoundsException     If a lazy option reads an unavailable option
     * @throws LogicException           If called from a lazy option or normalizer
     */
    public function resolve(array $options = array())
    {
        if ($this->locked) {
            throw new LogicException('Options cannot be resolved from a lazy option or normalizer.');
        }
        ...
        if (\count($diff) > 0) {
            ...
            throw new InvalidArgumentException(sprintf((\count($diff) > 1 ? 'The options "%s" do not exist.' : 'The option "%s" does not exist.') . ' Defined options are: "%s".', implode('", "', array_keys($diff)), implode('", "', array_keys($clone->defined))));
        }
        ...
        if (\count($diff) > 0) {
            ...
            throw new InvalidArgumentException(sprintf(\count($diff) > 1 ? 'The required options "%s" are missing.' : 'The required option "%s" is missing.', implode('", "', array_keys($diff))));
        }
     ...
     *
     * @throws LogicException           If accessing this method outside of
     *                                   {@link resolve()}
     * @throws OutOfBoundsException     If the option is not set
     * @throws InvalidArgumentException   If the option doesn't fulfill the
     *                                   specified validation rules
     * @throws LogicException If there is a cyclic dependency between
     *                                   lazy options and/or normalizers
     */
    ...
    {
        if (!$this->locked) {
            throw new LogicException('Array access is only supported within closures of lazy options and normalizers.');
        }
        ...
        if (!array_key_exists($option, $this->defaults)) {
            if (!isset($this->defined[$option])) {
                throw new OutOfBoundsException(sprintf('The option "%s" does not exist. Defined options are: "%s".', $option, implode('", "', array_keys($this->defined))));
            }

            throw new OutOfBoundsException(sprintf('The optional option "%s" has no value set. You should make sure it is set with "isset" before reading it.', $option));
        }
        ...
        if (isset($this->lazy[$option])) {
            ...
            if (isset($this->calling[$option])) {
                throw new LogicException(sprintf('The options "%s" have a cyclic dependency.', implode('", "', array_keys($this->calling))));
            }
        ...
        if (isset($this->allowedTypes[$option])) {
            ...
            if (!$valid) {
                throw new InvalidArgumentException(sprintf('The option "%s" with value %s is expected to be of type "%s", but is of type "%s".', $option, $this->formatValue($value), implode('" or "', $this->allowedTypes[$option]), $this->formatTypeOf($value)));
            }
        }
     ...
     *
     * @throws LogicException If accessing this method outside of {@link resolve()}
     *
    ...
    {
        throw new LogicException('Setting options via array access is not supported. Use setDefault() instead.');
    }
    ...
    {
        throw new LogicException('Removing options via array access is not supported. Use remove() instead.');
    }
    ...
    {
        if (!$this->locked) {
            throw new LogicException('Counting is only supported within closures of lazy options and normalizers.');
        }
